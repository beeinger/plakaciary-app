...
const Slider = class Slider extends React.Component {
  ...
  static slideSizeInPx(
    ...
    return (
      (orientation === "horizontal" ? sliderTrayWidth : sliderTrayHeight) /
      totalSlides
    );
  }
  ...
  static slidesMoved(
    ...
    const smallDrag =
      Math.abs(delta) >= slideSizeInPx * threshold ? dragStep : 0;
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!prevProps.isPageScrollLocked && this.props.isPageScrollLocked)
      this.lockScroll();
    if (prevProps.isPageScrollLocked && !this.props.isPageScrollLocked)
      this.unlockScroll();
  ...
  fakeOnDragStart({ screenX, screenY, touchDrag = false, mouseDrag = false }) {
    this.props.carouselStore.setStoreState({
      isPlaying: false,
    });
  ...
  play() {
    ...
    this.interval = setInterval(
      playDirection === "forward" ? this.playForward : this.playBackward,
      this.props.interval
    );
  }
  ...
  lockScroll() {
    ...
    if (this.scrollParent) {
      this.originalOverflow =
        this.originalOverflow || this.scrollParent.style.overflow;
  ...
  renderMasterSpinner() {
    ...
    if (hasMasterSpinner && !masterSpinnerFinished) {
      if (typeof this.props.onMasterSpinner === "function")
  ...
  render() {
    ...
    const trayWrapClasses = cn([
      ...
      orientation === "vertical"
        ? s.verticalSlideTrayWrap
        : s.horizontalTrayWrap,
      orientation === "vertical"
    ...
    const { dragStep, step, infinite, ...rest } = props;
